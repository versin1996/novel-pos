...
(function(global, setTimeout) {
        ...
        function removeScript(name) {
            if (isBrowser) {
                each(scripts(), function(scriptNode) {
                    if (scriptNode.getAttribute(
                            'data-requiremodule') === name &&
                        scriptNode.getAttribute(
                            'data-requirecontext') === context.contextName
                    ) {
                        scriptNode.parentNode.removeChild(
                            scriptNode);
                        return true;
                    }
                });
        ...
        function breakCycle(mod, traced, processed) {
            ...
            if (mod.error) {
                ...
                each(mod.depMaps, function(depMap, i) {
                    ...
                    if (dep && !mod.depMatched[i] && !processed[
                            depId]) {
        ...
        Module.prototype = {
            ...
            callPlugin: function() {
                ...
                on(pluginMap, 'defined', bind(this, function(plugin) {
                    var load, normalizedMap,
                        normalizedMod,
                        bundleId = getOwn(bundlesMap,
                            this.map.id),
                        name = this.map.name,
                        parentName = this.map.parentMap ?
                        this.map.parentMap.name : null,
                        localRequire = context.makeRequire(
                            map.parentMap, {
                                enableBuildCallback: true
                            });
                    ...
                    if (bundleId) {
                        this.map.url = context.nameToUrl(
                            bundleId);
                        this.load();
                        return;
                    }
                    ...
                    load.fromText = bind(this, function(
                        text, textAlt) {
                        /*jslint evil: true */
                    ...
                    plugin.load(map.name, localRequire,
                        load, config);
                }));
            ...
            enable: function() {
                ...
                //a dependency
                eachProp(this.pluginMaps, bind(this, function(
                    pluginMap) {
                    var mod = getOwn(registry,
                        pluginMap.id);
                    if (mod && !mod.enabled) {
                        context.enable(pluginMap, this);
                    }
        ...
        context = {
            ...
            makeShimExports: function(value) {
                function fn() {
                    ...
                    return ret || (value.exports && getGlobal(value
                        .exports));
                }
                return fn;
            },
            ...
            completeLoad: function(moduleName) {
                ...
                if (!found && !hasProp(defined, moduleName) && mod &&
                    !mod.inited) {
                    if (config.enforceDefine && (!shExports || !
                            getGlobal(shExports))) {
    ...
    req = requirejs = function(deps, callback, errback, optional) {
        ...
        if (!context) {
            context = contexts[contextName] = req.s.newContext(
                contextName);
        }
